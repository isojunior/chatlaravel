					...
					<li class="list-group-item link">
				    	<div class="row">
					    	...
					    	<div class="col-xs-8">
								...
								<div class="row">
									<button onclick="cancelAuthorized({{ $hiUser['ID_USER'] }})" class="btn btn-danger btn-sm cancel">ยกเลิกเป็นผู้เกี่ยวข้อง</button>
								</div>
							</div>
				    	</div>
				    </li>
						...
						<li class="list-group-item link">
					    	<div class="row">
						    	...
						    	<div class="col-xs-9">
									...
									<button onclick="cancelAuthorized({{ $nmUser['ID_USER'] }})" class="btn btn-danger btn-sm cancel">ยกเลิกเป็นผู้เกี่ยวข้อง</button>
						    	</div>
					    	</div>
					    </li>
				@endforeach
					...
					@foreach ($unAuthorize as $unAuth)
							<<li class="list-group-item link">
						    	...
						    	<div class="col-xs-9">
									...
									<button onclick="addToAuthorized({{ $unAuth['ID_USER'] }})" data-attr="{{ $unAuth['ID_USER'] }}" class="btn btn-success btn-sm accept">เพิ่มเป็นผู้เกี่ยวข้อง</button>
									<button onclick="addToGroup({{ $unAuth['ID_USER'] }})" data-attr="{{ $unAuth['ID_USER'] }}" class="btn btn-info btn-sm group">เพิ่มเข้ากลุ่ม</button>
									<button onclick="reject({{ $unAuth['ID_USER'] }})" data-attr="{{ $unAuth['ID_USER'] }}" class="btn btn-warning btn-sm reject">ปฏิเสธ</button>
						    	</div>
					    	</div>
					    </li>
				@endforeach
						...
						<li href="#" class="list-group-item link">
					    	<div class="row">
						    	...
						    	<div class="col-xs-9">
									...
									<button onclick="addToAuthorized({{ $reUser['ID_USER'] }})" data-attr="{{ $reUser['ID_USER'] }}" class="btn btn-success btn-sm accept">เพิ่มเป็นผู้เกี่ยวข้อง</button>
									<button onclick="addToGroup({{ $reUser['ID_USER'] }})" data-attr="{{ $reUser['ID_USER'] }}"class="btn btn-info btn-sm group">เพิ่มเข้ากลุ่ม</button>
						    	</div>
					    	</div>
					    </li>
				@endforeach
